...
void MainWindow::DisplayError(uint16_t error)
{
    ...
    case ERR_NO_FREE_SPACE:{
        ui->tb_result->append("На сервере не осталось места");
    }
    case ERR_NO_FUNCT:
    default:
        break;
...
{   
    switch (typeMess) {
    case CLEAR_DATA:{
        ui->tb_result->append("Данные на сервере, успешно удалены, стёрты из памяти, и забыты");
    }
    default:
        break;
    }
...
void MainWindow::on_pb_request_clicked()
{
   ...
   if(ui->cb_request->currentIndex() == 3) client->SendData(header, ui->le_data->text());
   else client->SendRequest(header);
}
